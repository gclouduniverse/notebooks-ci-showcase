...
function execute_notebook_with_gpu() {
    ...
    gcloud compute instances create "${INSTANCE_NAME}" \
            ...
            --metadata="api_key=${API_KEY},input_notebook=${INPUT_NOTEBOOK_GCS_PATH},output_notebook=${OUTPUT_NOTEBOOK_GCS_FOLDER}${PARAM_METADATA:-},startup-script=https://raw.githubusercontent.com/gclouduniverse/gcp-notebook-executor/master/notebook_executor.sh" \
            --quiet
    if [[ $? -eq 1 ]]; then
