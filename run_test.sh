...
function execute_notebook_with_gpu() {
    ...
    gcloud compute instances create "${INSTANCE_NAME}" \
            ...
            --metadata="testing_mode=true,input_notebook=${INPUT_NOTEBOOK_GCS_PATH},output_notebook=${TEMP_GCS_LOCATION},startup-script-url=https://raw.githubusercontent.com/b0noI/gcp-notebook-executor/master/notebook_executor.sh" \
            --quiet
    if [[ $? -eq 1 ]]; then
